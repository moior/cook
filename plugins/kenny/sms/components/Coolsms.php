<?php namespace Kenny\Sms\Components;
...
class Coolsms
{
    ...
    private $api_key = 'NCS56D7D558B75F2';
    private $api_secret = 'FF66814691163D7E6E3DA2B3EE6172FA';
    ...
    public function onRun($basecamp=false) //__construct
    {
        if($basecamp)
        {
            $this->coolsms_user = $this->api_key;
            $this->basecamp = true;
        }
        /*else
    ...
    private function addInfos($options)
    {
        ...
        if(empty($options->User_Agent)) $options->User_Agent = sprintf("PHP REST API %s", $this->version);
        if(empty($options->os_platform)) $options->os_platform = $this->getOS();
        if(empty($options->dev_lang)) $options->dev_lang = sprintf("PHP %s", phpversion());
        if(empty($options->sdk_version)) $options->sdk_version = sprintf("PHP SDK %s", $this->sdk_version);
